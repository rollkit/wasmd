...
CHAIN_ID=rosm
KEY_NAME=rosm-gen
TOKEN_AMOUNT="10000000000000000000000000urosm"
STAKING_AMOUNT=1000000000urosm
CHAINFLAG="--chain-id ${CHAIN_ID}"
TXFLAG="--chain-id ${CHAIN_ID} --gas-prices 0rosm --gas auto --gas-adjustment 1.3"
...
#AUTH_TOKEN=$(docker exec $(docker ps -q) celestia light auth write --p2p.network mocha)
...
sed -i'' -e 's/^minimum-gas-prices *= .*/minimum-gas-prices = "0urosm"/' "$HOME"/.wasmd/config/app.toml
...
sed -i'' -e 's/bond_denom": ".*"/bond_denom": "urosm"/' "$HOME"/.wasmd/config/genesis.json
sed -i'' -e 's/mint_denom": ".*"/mint_denom": "urosm"/' "$HOME"/.wasmd/config/genesis.json
...
echo "wasmd start --rollkit.aggregator --rollkit.da_auth_token=\$AUTH_TOKEN --rollkit.da_namespace 00000000000000000000000000000000000000000008e5f679bf7116cb --rollkit.da_start_height \$DA_BLOCK_HEIGHT --rpc.laddr tcp://0.0.0.0:36657 --grpc.address 0.0.0.0:9290 --p2p.laddr \"0.0.0.0:36656\" --minimum-gas-prices=\"0.025urosm\" --rollkit.da_gas_multiplier 1.2 --rollkit.block_time 10s" >> restart-wasmd.sh
